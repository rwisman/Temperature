...
void BleepMachine::queueCallback( AudioQueueRef outAQ, AudioQueueBufferRef outBuffer )
{
	...
	float fStep = wave.fStep;
	
    for( int s=0; s<kBufferSizeInFrames; s++ )
    {
        float sample = ( wave.volume * sinf( phase ) );
    ...
    wave.phase = fmodf( phase, 2 * M_PI );   // Take modulus to preserve precision
	
    // Enqueue the buffer
    AudioQueueEnqueueBuffer( m_outAQ, outBuffer, 0, NULL ); 
}

bool BleepMachine::SetWave( float frequency, float volume )
{
    wave.volume = volume;
...
bool BleepMachine::Initialise()
{
    ...
    m_outFormat.mChannelsPerFrame = 1;
    m_outFormat.mBytesPerPacket = m_outFormat.mBytesPerFrame = sizeof(UInt16);
...
BleepMachine::BleepMachine() : m_isInitialised(false), m_outAQ(0)
{
    for ( int buf=0; buf<kNumBuffers; buf++ ) 
    {
        m_buffers[ buf ] = NULL;
...
bool BleepMachine::Stop()
{
    ...
    if ( result ) printf( "BleepMachine ERROR: %d\n", (int)result );
